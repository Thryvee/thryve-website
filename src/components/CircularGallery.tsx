...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: Screen; viewport?: Viewport } = {}) {
    ...
    if (viewport) {
      this.viewport = viewport;
      const uViewportSizes = (this.plane.program.uniforms as Record<string, { value: unknown }>)
        .uViewportSizes;
      if (uViewportSizes) {
...
class App {
  ...
  autoAdvance(amount: number) {
    this.scroll.target += amount;
    this.totalScrolled += amount;
    if (!this.loopFired && this.medias[0] && this.totalScrolled >= this.medias[0].widthTotal / 2) {
      this.loopFired = true;
      this.onLoopComplete?.();
    }
  }
  ...
  function CircularGallery(
    ...
    useImperativeHandle(ref, () => ({
      advanceBy: (amount: number) => {
        appRef.current?.autoAdvance(amount);
      },
    }));
