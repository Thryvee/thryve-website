...
function getLocalPointerPos(e: PointerLike, rect: DOMRect) {
  let clientX = 0,
    clientY = 0;
...
class ImageTrailVariant1 {
  ...
  constructor(container: HTMLElement) {
    ...
    const handlePointerMove = (ev: PointerLike) => {
      const rect = this.container.getBoundingClientRect();
      this.mousePos = getLocalPointerPos(ev, rect);
    };
    container.addEventListener("mousemove", handlePointerMove as unknown as EventListener);
    container.addEventListener("touchmove", handlePointerMove as unknown as EventListener);

    const initRender = (ev: PointerLike) => {
      const rect = this.container.getBoundingClientRect();
      this.mousePos = getLocalPointerPos(ev, rect);
      ...
      container.removeEventListener("mousemove", initRender as unknown as EventListener);
      container.removeEventListener("touchmove", initRender as unknown as EventListener);
    };
    container.addEventListener("mousemove", initRender as unknown as EventListener);
    container.addEventListener("touchmove", initRender as unknown as EventListener);
  }
...
class ImageTrailVariant7 {
  ...
  showNextImage() {
    ...
    gsap.killTweensOf(img.DOM.el);
    const scaleValue = gsap.utils.random(0.5, 1.6) as number;
    ...
    gsap
      ...
      .fromTo(
        img.DOM.el,
        {
          scale: scaleValue - Math.max(gsap.utils.random(0.2, 0.6) as number, 0),
          rotationZ: 0,
          opacity: 1,
          zIndex: this.zIndexVal,
          x: this.cacheMousePos.x - img.rect.width / 2,
...
const variantMap: Record<number, new (container: HTMLElement) => TrailInstance> = {
