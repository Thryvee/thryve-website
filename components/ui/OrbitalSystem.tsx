...
const PILLARS = [
  { name: 'Acquire',  metric: '2x ROAS',     color: '#9B5DE5', radius: 110, speed: 0.008,  size: 10, startAngle: 0 },
  { name: 'Convert',  metric: '+200% CVR',    color: '#4361EE', radius: 160, speed: 0.005,  size: 9,  startAngle: Math.PI / 2 },
  { name: 'Retain',   metric: '3:1 LTV:CAC',  color: '#2DC653', radius: 215, speed: 0.0035, size: 11, startAngle: Math.PI },
  { name: 'Scale',    metric: '60% less time', color: '#F4A261', radius: 270, speed: 0.002,  size: 9,  startAngle: (3 * Math.PI) / 2 },
];
...
export default function OrbitalSystem() {
  ...
  const trailsRef = useRef(PILLARS.map(() => [] as { x: number; y: number; alpha: number }[]));
  ...
  useEffect(() => {
    ...
    const pillars = PILLARS.map(p => ({ ...p, radius: p.radius * scale, size: p.size * scale }));
    ...
    const draw = () => {
      ...
      pillars.forEach((p, i) => {
        ...
        // Planet shine
        ctx.beginPath();
        ctx.arc(px - planetSize * 0.3, py - planetSize * 0.3, planetSize * 0.35, 0, Math.PI * 2);
